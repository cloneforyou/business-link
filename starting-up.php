<!DOCTYPE html>
<html class="no-js">
<head>
	<meta charset="utf-8">	
	<title>Starting Up | Business Link</title>
	<meta name="author" content="Caroline Murphy">
	<meta name="viewport" content="width=device-width">
</head>

<body>

<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/header.php'; ?>

<div id="content" > 
	
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/breadcrumbs.php'; ?>

	<div id="layer11">
		<div id="maincontent">
			<h1>Starting up</h1>
			<div id="theme_container">
				<div id="theme_title">
					<h2>My New Business</h2>
				</div>
				<div id="theme_left">
					<p><img src="/_img/startingupabusiness.jpg" alt="" /> </p>
				</div>
				<div id="theme_right">
					<h3>Start your business here</h3>
					<ul id="start_up_list">
						<li>Over 200 new video and learning resources</li>
						<li>Create your own checklist from our recommended start-up tasks</li>
						<li>Access government services online</li>
					</ul>
				</div>
				<div id="newBusinessContent">
					<a title="Go to My New Business" href="https://www.gov.uk/browse/business/setting-up" rel="external">Get started</a>
				</div>
			</div>
			
			<h2>Before you start</h2>
			<ul>
				<li><a href="/starting-up/considering-starting-up.php">Considering starting up?</a></li>
				<li><a href="/starting-up/business-planning.php">Business planning</a></li>
				<li><a href="/starting-up/find-and-manage-the-money.php">Find and manage the money</a></li>
			</ul>
			<h2>Forming and naming your business</h2>
			<ul>
				<li><a href="/starting-up/business-structures.php">Business structures</a></li>
				<li><a href="/starting-up/name-your-business.php">Name your business</a></li>
				<li><a href="/starting-up/buy-a-business.php">Buy a business</a></li>
			</ul>
			<h2>Your workplace</h2>
			<ul>
				<li><a href="/starting-up/choose-and-set-up-your-workplace.php">Choose and set up your workplace</a></li>
			</ul>
			<h2>Tax, payroll &amp; company information</h2>
			<ul>
				<li><a href="https://www.gov.uk/browse/tax" title="Money and tax" rel="external">Get to grips with business taxes</a></li>
				<li><a href="/starting-up/tax-and-national-insurance-for-self-employed-and-partners.php">Tax and National Insurance for self-employed or partners</a></li>
				<li><a href="/starting-up/corporation-tax.php">Corporation Tax</a></li>
				<li><a href="/starting-up/vat.php">VAT</a></li>
				<li><a href="/starting-up/tax-and-payroll-for-employers.php">Tax and payroll for employers</a></li>
			</ul>
			<h2>Employing people</h2>
			<ul>
				<li><a href="starting-up/become-an-employer-for-the-first-time.php">Become an employer for the first time</a></li>
			</ul>
			<h2>Managing your business</h2>
			<ul>
				<li><a href="/starting-up/sales-and-marketing.php">Sales and marketing</a></li>
				<li><a href="/starting-up/it-and-ecommerce-for-startups.php">IT and e-commerce for start ups</a></li>
				<li><a href="/starting-up/trade-with-other-countries.php">Trade with other countries</a></li>
				<li><a href="/starting-up/protect-your-business-ideas.php">Protect your business ideas</a></li>
			</ul>
			<h2>Help and support</h2>
			<ul>
				<li><a href="/starting-up/further-support-for-your-business.php">Further support for your business</a></li>
			</ul>
			<h2>Go for growth</h2>
			<ul>
				<li><a href="/starting-up/take-the-next-steps.php">Take the next steps</a></li>
			</ul>
			<p><a href="https://www.gov.uk/browse/business/setting-up" rel="external">Starting up - site map</a></p>
		</div>

		<div id="additionalcontent">
			<div>
				<h3>Spotlight</h3>
				<div>
					<div>
						<h4><a href="find-the-business-in-you.php">The business in you</a></h4>
						<p>Starting and growing a business - tips and tools from real-life success stories</p>
					</div>
					<div>
						<a href="find-the-business-in-you.php" title="The 'Business in You' is a major new campaign to help people to start a new business or grow their existing one" >
							<img src="/_img/Businessinyou_whatsnewpromo.jpg" width="76" height="76" alt="The "business in you" is a major new campaign to help people to start a new business or grow their existing one" />
						</a>
					</div>
				</div>
				<div>
					<div>
						<h4><a href="do-it-online-register-online-for-business-taxes.php">HMRC registration service</a></h4>
						<p>Need to register for business taxes? Use the new HMRC tax registration service</p>
					</div>
					<div>
						<a href="do-it-online-register-online-for-business-taxes.php" title="Register online for business taxes">
							<img src="/_img/Businessman.jpg" width="76" height="76" alt="Register online for business taxes" />
						</a>
					</div>
				</div>
				<div>
					<div>
						<h4><a href="https://www.gov.uk/business-selling-customers" rel="external">Understand your market</a></h4>
						<p>How to find out about customers and competitors</p>
					</div>
					<div>
						<a href="https://www.gov.uk/business-selling-customers" title="Researching your market and competition">
							<img src="/_img/Understand_your_market.jpg" width="76" height="76" alt="Researching your market and competition" />
						</a>
					</div>
				</div>
				<div>
					<div>
						<h4><a href="https://www.gov.uk/business-finance-explained" rel="external">How much money do you need?</a></h4>
						<p>Work out the cost of starting and running a new business</p>
					</div>
					<div>
						<a href="https://www.gov.uk/business-finance-explained" title="Identify your start-up costs and manage your money. Find out how to forecast your cashflow and record your profit and loss">
							<img src="/_img/How_much_money_do_you_need.jpg" width="76" height="76" alt="Identify your start-up costs and manage your money. Find out how to forecast your cashflow and record your profit and loss" />
						</a>
					</div>
				</div>
				<div>
					<div>
						<h4><a href="https://www.gov.uk/starting-up-a-business" rel="external">Be your own boss</a></h4>
						<p>Decide if you've got what it takes</p>
					</div>
					<div>
						<a href="https://www.gov.uk/starting-up-a-business" title="Learn about the qualities you need to be your own boss and how you can use your skills and experience to get your business up and running">
							<img src="/_img/Be_your_own_boss.jpg" width="76" height="76" alt="Learn about the qualities you need to be your own boss and how you can use your skills and experience to get your business up and running" />
						</a>
					</div>
				</div>
				<div>
					<div>
						<h4><a href="https://www.gov.uk/business-legal-structures" rel="external">Your business options</a></h4>
						<p>Understand the different business types</p>
					</div>
					<div>
						<a href="https://www.gov.uk/business-legal-structures" title="Learn about the different types of business and legal structures to help you choose the right one for your new business">
							<img src="/_img/Your_business_options.jpg" width="76" height="76" alt="Learn about the different types of business and legal structures to help you choose the right one for your new business" />
						</a>
					</div>
				</div>
				<div>
					<div>
						<h4><a href="do-it-online-register-online-for-business-taxes.php" rel="external">Your business type</a></h4>
						<p>Choose the right business model and legal structure</p>
					</div>
					<div>
						<a href="do-it-online-register-online-for-business-taxes.php" title="Tutorials, videos and tools to help you decide how to become self-employed - from sole trader to limited company, franchisee to social enterprise and more" >
							<img src="/_img/Your_business_type.jpg" width="76" height="76" alt="Tutorials, videos and tools to help you decide how to become self-employed - from sole trader to limited company, franchisee to social enterprise and more" />
						</a>
					</div>
				</div>
				<div>
					<div>
						<h4><a href="https://www.gov.uk/write-business-plan" rel="external">Create your business plan</a></h4>
						<p>All you need to know to write your plan</p>
					</div>
					<div>
						<a href="https://www.gov.uk/write-business-plan" title="What you need to include in your business plan and how to create one step-by-step" rel="external">
							<img src="/_img/Create_your_business_plan.jpg" width="76" height="76" alt="What you need to include in your business plan and how to create one step-by-step" />
						</a>
					</div>
				</div>
			</div>
			<div>
				<h3>Do it online</h3>
				<ul>
					<li><a href="https://www.gov.uk/get-information-about-a-company" rel="external">Search for an available company name and trade mark</a></li>
					<li><a href="do-it-online-incorporate-your-company-online.php">Incorporate your company online</a></li>
					<li><a href="do-it-online-apply-to-register-a-trademark.php">Apply to register a trade mark</a></li>
				</ul>
			</div>
			<div>
				<h3>Directories</h3>
				<ul>
					<li><a href="https://www.gov.uk/browse/business/setting-up" title="National and local events, seminars and workshops in your area to help you learn or network" rel="external">Starting up events near you</a></li>
					<li>
						<a href="browse-by-theme-starting-up.php">Starting up case studies</a>
					</li>
				</ul>
			</div>
		</div>
	</div>

<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/footer.php'; ?>