<!DOCTYPE html>
<html>
<head>
	<title>Commercial property: responsibilities of business tenants | Business Link</title>
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/meta.php'; ?>

	<!-- ref: layer8aba.html || /layer?topicId=1074458562 -->
	<meta name="description" content="How tenants are responsible for health and safety and maintenance duties in commercial properties" />
	<meta name="keywords" content="Health and safety duties of commercial premises tenants; tenancy agreement,gas safety regulations,CORGI,fire precautions,fire risk assessment,fire regulations,fire certificate,electrical safety,manage asbestos,asbestos containing materials,maintain premises,dilapidations,ACMs,preventative maintenance; Business practice and regulation;Occupational health and safety;Fire;Small businesses;energy performance certificate;EPC;Gas Safety Register" />
</head>

<body>
<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/header.php'; ?>

<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/breadcrumbs.php'; ?>

<div class="row">
	<div class="small-12 large-8 large-push-4 columns">
		
		<h1>Commercial property: responsibilities of business tenants</h1>
		<h2>Introduction</h2>
		<img src="/_img/HS0750.gif" width="94" height="62" alt="" />
		<p>If you are the tenant of commercial premises, it's important to be aware of the responsibilities you have for maintaining the premises and for the health and safety of occupants - particularly in areas such as gas and fire safety.</p>
		<p>Some responsibilities are laid down by statute and others have the force of law because they are set out in a lease.</p>
		<p>In some areas these responsibilities will be solely yours. In others they may be shared with your landlord.</p>
		<p>This guide explains how these important responsibilities are divided, either by the law or by the terms of the lease.</p>
		
		<ul>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/general-health-and-safety-responsibilities.php" title="General health and safety responsibilities">Next</a></li>
		</ul>
		
		<h3>Subjects covered in this guide</h3>
		<ul>
			<li>Introduction</li>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/general-health-and-safety-responsibilities.php">General health and safety responsibilities</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/ensuring-gas-safety.php">Ensuring gas safety</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/ensuring-fire-safety.php">Ensuring fire safety</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/electrical-safety.php">Electrical safety</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/duty-to-manage-asbestos-in-premises.php">Duty to manage asbestos in premises</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/energy-performance.php">Energy performance</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/commercial-property-responsibilities-of-business-tenants/responsibilities-for-maintenance-of-commercial-properties.php">Responsibilities for maintenance of commercial properties</a></li>
		</ul>
		
		<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/print-email.php'; ?>
					
		<h3>RICS Enquiry Line</h3>
		<p>0870 333 1600</p>
		<hr />
		
		<h3>Actions</h3>
		<ul>
			<li><a href="http://www.leasingbusinesspremises.co.uk/downloads/occupier_guide.pdf" target="bgExternalonlinebusinesslinkgovuk" title="Guidance for occupiers of business rental property">Download the occupier guide from the Leasing Business Premises website (PDF, 806K)</a></li>
			<li><a href="https://www.hse.gov.uk/simple-health-safety/index.htm" target="bgExternalonlinebusinesslinkgovuk" title="How to comply with the law and manage health and safety in your business">Health and safety made simple on the Health &amp; Safety Executive website</a></li>
		</ul>

		<p>Developed with:<br />
			<a href="https://www.hse.gov.uk/" title="Health and Safety Executive" target="_blank" rel="noopener noreferrer">
				<img src="/_img/HSE_logo.jpg" width="61" height="61" alt="" /> Health and Safety Executive
			</a>
			<br />
			<a href="https://www.gov.uk/government/organisations/department-for-communities-and-local-government" title="Communities and Local Government" target="_blank" rel="noopener noreferrer">
				<img src="/_img/communities_local_governmen.jpg" width="103" height="35" alt="" /> Communities and Local Government
			</a>
		</p>
		
		<img src="/_img/px_healthandenvironment.gif" alt="" width="181" height="5" />

		
	</div><!-- // .small-12 large-8 large-push-4 columns -->
	
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/category-links.php'; ?>

</div><!-- // .row -->

<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/footer.php'; ?>