<!DOCTYPE html>
<html>
<head>
	<title>Use your home as a workplace | Business Link</title>
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/meta.php'; ?>

	<!-- ref: layer84d1.html || /layer?topicId=1073960216 -->
	<meta name="description" content="Tax implications, financial and legal issues, security and setting up your work area if you plan to work from home" />
	<meta name="keywords" content="working from home; regional homeworking projects;work life balance;domestic workplace;homeworking;work from home;relocation;working from home;telework;teleworkers;homeworkers;home office;flexible working;home-based business; Occupational health and safety" />
</head>

<body>
<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/header.php'; ?>

<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/breadcrumbs.php'; ?>

<div class="row">
	<div class="small-12 large-8 large-push-4 columns">
		
		<h1>Use your home as a workplace</h1>
		<h2>Introduction</h2>
		<img src="/_img/HS0290.gif" width="78" height="118"  alt="" />
		<p>If you are planning to set up and run a 'work at home' business, there are a number of points to check before you begin.</p>
		<p>Working from home can affect your mortgage, your home insurance, your tax situation, other people living in your house and even your neighbours.</p>
		<p>This guide explains the advantages and disadvantages of working from home and helps you turn the place you live into a place of work.</p>
		
		<ul>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/what-to-check-before-starting-a-work-at-home-business.php" title="What to check before starting a work at home business">Next</a></li>
		</ul>
		
		<h3>Subjects covered in this guide</h3>
		<ul>
			<li>Introduction</li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/what-to-check-before-starting-a-work-at-home-business.php">What to check before starting a work at home business</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace.php">Building regulations and planning permission for home businesses</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/tax-considerations-of-working-from-home.php">Tax considerations of working from home</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/home-working-health-and-safety-risk-assessment.php">Home working health and safety risk assessment</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/create-a-seperate-work-area-in-your-home.php">Create a seperate work area in your home</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/work-equipment-and-workstation-setup.php">Work equipment and workstation setup</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/advantages-and-disadvantages-of-working-from-home.php">Advantages and disadvantages of working from home</a></li>
			<li><a href="/premises-and-property/choosing-the-right-premises/use-your-home-as-a-workplace/here-is-how-I-ensured-that-my-home-was-safe-for-use-as-a-workplace.php">Here's how I ensured that my home was safe for use as a workplace</a></li>
		</ul>
		
		<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/print-email.php'; ?>

		<h3>Actions</h3>
		<ul>
			<li><a href="/_files/hse_homeworkers-guidance-for-employers-on-health-and-safety_08-11.pdf" target="_blank" rel="noopener noreferrer">Download homeworking information from the Health &amp; Safety Executive website (PDF, 119K)</a></li>
			<li><a href="/finance-and-grants/insurance/get-the-right-insurance-for-your-business.php">Use our interactive tool to work out which forms of insurance you should have for your business</a></li>
		</ul>
		
		<p>Developed with:<br />
			<a href="https://www.hse.gov.uk/" title="Health and Safety Executive" target="_blank" rel="noopener noreferrer">
				<img src="/_img/HSE_logo.jpg" width="61" height="61" alt="" /> Health and Safety Executive
			</a>
		</p>

		<img src="/_img/px_healthandenvironment.gif" alt="" width="181" height="5" />

		
	</div><!-- // .small-12 large-8 large-push-4 columns -->
	
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/category-links.php'; ?>

</div><!-- // .row -->

<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/footer.php'; ?>