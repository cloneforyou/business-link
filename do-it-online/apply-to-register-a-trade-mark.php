<!DOCTYPE html>
<html class="no-js">

<head>
	<title>Apply to register a trade mark | Business Link</title>
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/meta.php'; ?>
	<!-- ref: detail05ac.html || /detail?itemId=1082501507&type=PIP -->
</head>

<body>
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/header.php'; ?>
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/breadcrumbs.php'; ?>
	<div class="row">
		<div class="small-12 large-8 large-push-4 columns">
			<h1>Apply to register a trade mark</h1>
			<p>Once you have chosen a trade mark you would like to use for your business, goods or services, you should apply to register it with the Intellectual Property Office. Remember that your trade mark may not be accepted if it describes your goods and services in any way. Once registered, your trade mark will be legally protected within the UK for a period of 10 years.</p>
			<p>It is advisable to carry out a search for identical and similar trade marks before applying. You may apply to register a trade mark that is the same as, or similar to, an existing one, but the owner may oppose the new application.</p>
			<p>Trade marks also need to be classified by the specific goods and services they will be applied to. A full list of goods and services, together with the appropriate class numbers, is available on the Intellectual Property Office website.</p>
			<p>To apply to register your trade mark, complete the online application form on the Intellectual Property Office website by clicking the button below. Please note there is a fee to pay which will vary according to the filing option you choose. (All major credit/debit cards are accepted. Mastercard Secure Pay and Verified by Visa are used to improve online card payment security.)</p>
			<p>While completing the online form, you will be able to save and return to it. Please note the service is unavailable between 00.15 and 03.15 every morning.</p>
			
			<p><a href="https://trademarks.ipo.gov.uk/ipo-apply" target="_blank" rel="noopener noreferrer">Apply to register a trade mark</a></p>

			<p>In partnership with:<br />
				<a href="https://www.gov.uk/government/organisations/intellectual-property-office" target="_blank" rel="noopener noreferrer" title="Intellectual Property Office">
					<img src="/_img/IPO_LOGO.gif" width="150" height="26" alt="" /> Intellectual Property Office
				</a>
			</p>
			
		</div><!-- // .small-12 large-8 large-push-4 columns -->
		<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/category-links.php'; ?>
	</div><!-- // .row -->
	<?php include $_SERVER['DOCUMENT_ROOT'].'/_inc/footer.php'; ?>